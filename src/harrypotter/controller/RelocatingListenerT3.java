...
public class RelocatingListenerT3 extends JFrame implements ActionListener {
	...
	public void setThirdTask(ThirdTask thirdTask) {
		ThirdTask = thirdTask;
	}
	...
	private useSpellListenerT3 useSpellListenerT3;
	private Direction Directionx;
	private JList List1;
	private JList List2;
	
	public  RelocatingListenerT3(ThirdTask ThirdTask, Tournament Tournament,HarryPotterView HarryPotterView, useSpellListenerT3 useSpellListenerT3 ){
		this.ThirdTask=ThirdTask;
		this.Tournament=Tournament;
		this.HarryPotterView=HarryPotterView;
		this.useSpellListenerT3=useSpellListenerT3;
	...
	public void actionPerformed(ActionEvent e) {
		generatemapT3();
		//JOptionPane.showMessageDialog(null, ((JReferencingButton) e.getSource()).getValue()+" "+FirstTask.getTargetPoint(Direction.FORWARD ));
		int x=(int) ((Point) ((JReferencingButton) e.getSource()).getValue()).getX();
		int y=(int) ((Point) ((JReferencingButton) e.getSource()).getValue()).getY();
		if(x==ThirdTask.getTargetPoint(Direction.FORWARD).getX()&& y==ThirdTask.getTargetPoint(Direction.FORWARD).getY()){
			setDirectionx(Direction.FORWARD);
			//useSpellListener.remove(useSpellListener.getSpells());
			JLabel Area = new JLabel(" Select range and position");
			Font font = new Font("Courier", Font.BOLD,12);
			Area.setFont(font);
			JPanel target = new JPanel();
			target.setLayout(new GridLayout());
			target.add(Area,BorderLayout.PAGE_START);
			String[] data= new String[((RelocatingSpell) useSpellListenerT3.getJReferencingButton()).getRange()+1];
			for(int i =0;i<=((RelocatingSpell) useSpellListenerT3.getJReferencingButton()).getRange();i++){
				data[i]=String.valueOf(i);
			}
			 List1=new JList(data);
			String text [] = {"BACKWARD","FORWARD","LEFT","RIGHT"};
			 List2=new JList(text);
			JButton m = new JButton("OK");
			m.addActionListener(new Relocate2ListenerT3(ThirdTask, Tournament, HarryPotterView, useSpellListenerT3, this));
			target.add(List1);
			target.add(List2);
			target.add(m,BorderLayout.PAGE_END);
		setBounds(500, 500,500, 500);
			target.setPreferredSize(new Dimension(700, 700));
			add(target);
	     add(target);
	       pack();
	      ...
	      add(target);
		...
		else if(x==ThirdTask.getTargetPoint(Direction.BACKWARD).getX()&& y==ThirdTask.getTargetPoint(Direction.BACKWARD).getY()){
			setDirectionx(Direction.BACKWARD);
			//useSpellListener.remove(useSpellListener.getSpells());
			JLabel Area = new JLabel(" Select range and position");
			Font font = new Font("Courier", Font.BOLD,12);
			Area.setFont(font);
	       ...
	       pack();
		...
		else if(x==ThirdTask.getTargetPoint(Direction.LEFT).getX()&& y==ThirdTask.getTargetPoint(Direction.LEFT).getY()){
			setDirectionx(Direction.LEFT);
			JTextArea Area = new JTextArea(" Select range and position");
			Font font = new Font("Courier", Font.BOLD,12);
			Area.setFont(font);
			JPanel target = new JPanel();
			target.setLayout(new GridLayout());
		...
		else if(x==ThirdTask.getTargetPoint(Direction.RIGHT).getX()&& y==ThirdTask.getTargetPoint(Direction.RIGHT).getY()){
			...
			target.add(m,BorderLayout.PAGE_END);
			m.addActionListener(new Relocate2ListenerT3(ThirdTask, Tournament, HarryPotterView, useSpellListenerT3, this));
			//useSpellListener.setBounds(500, 500,200, 200);
			target.setPreferredSize(new Dimension(700, 700));
			add(target);
	        add(target);
	        pack();
			...
			JOptionPane.showMessageDialog(null, "you have to select a Cell near you");
			generatemapT3();
		}
