...
public class SecondTask extends Task {
	...
	public void encounterMerPerson() {
		...
		if (currentX + 1 <= 9
			...
			dmg=dmg+((Merperson) (cell.getObstacle())).getDamage();
			i++;
		...
		if(i !=0){
		JOptionPane.showMessageDialog(null, current.getName()+"was attacked by "+i+" Merpersons with Damage of "+dmg+" and his HP is "+newHp);
		}

		if (newHp <= 0) {
			JOptionPane.showMessageDialog(null, current.getName()+"was killed by Merpersons");
